...
- **`LastAltitude` stopped being a cruise proxy** the moment tracks began running to landing. It now reports a touchdown altitude and collapsed the analytics altitude-band chart into the bottom bucket. Hence the `MaxAltitude` column, computed at flight close over the *untrimmed* track. **The `MaxAltitude ?? LastAltitude` fallback this ADR expected to carry the chart did not hold.** Rows predating the migration kept falling back to a touchdown reading, 56% of the window landed in the bottom band, and the chart was dropped rather than repaired in [ADR-013](013-analytics-presentation.md). `MaxAltitude` is still populated and is still the right column for anything cruise-shaped, but nothing reads it today.
